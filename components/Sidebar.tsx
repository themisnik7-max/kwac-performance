...
export default function Sidebar({ active, role }: { active?: string; role?: string }) {
  ...
  // Auto-detect active from pathname if not provided
  ...
  return (
    <aside style={{
      ...
      <div style={{ padding: '0 1.25rem 1.5rem', borderBottom: '0.5px solid #333', marginBottom: '0.5rem' }}>
        ...
        {NAV.filter(n => !n.ceoOnly || isCeo).map(n => {
          ...
          return (
            <a key={n.key} href={n.href} style={{
              ...
              <span style={{ fontSize: 14, minWidth: 16 }}>{n.icon}</span>
              {n.label}
            </a>
          )
        })}
