...
export default function Sidebar({isCEO, onCEOToggle}) {
  ...
  return (
    <div style={{width:64,background:'#111',display:'flex',flexDirection:'column',alignItems:'center',paddingTop:16,paddingBottom:16,gap:2,position:'fixed',top:0,left:0,height:'100vh',zIndex:100,overflowY:'auto'}}>
      <div style={{color:'#fff',fontWeight:800,fontSize:12,marginBottom:16,letterSpacing:1,textAlign:'center',lineHeight:1.2,flexShrink:0}}>
      ...
      {NAV.map((it,i) => (
        <a key={i} href={it.href} style={{
          ...
          fontSize:15,textDecoration:'none',transition:'all .15s'
        }}>
          <span>{it.icon}</span>
      ...
      <a href="/login" style={{
        width:44,height:44,borderRadius:10,flexShrink:0,
        border:'1px solid rgba(255,255,255,.08)',marginTop:4,
        display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',
        gap:2,background:'transparent',color:'rgba(255,255,255,.3)',fontSize:13,textDecoration:'none'
      }}>
