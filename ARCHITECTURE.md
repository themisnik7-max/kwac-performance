...
3. **Pricing engine** (`app/api/meeting-valuation`) — a three-signal blend, all three logged per-run in `valuation_calibration_log`:
   - **Comp-based** (`lib/valuation.ts`) — deterministic, not ML. Comp-weighted average (time-decayed, IQR outlier-filtered) × floor/condition/age multipliers. No black box; every multiplier is a named, inspectable function.
   - **Small neural net** (`lib/miniNet.ts` + `lib/pricingFeatures.ts`) — a from-scratch, dependency-free MLP (2 hidden layers, ~35 input features: sqm, floor, age, renovated, condition, balcony, utilization_score, distance to metro/sea/Acropolis, competing active listings, one-hot neighborhood). Not an LLM, not a framework black box — every weight is plain JSON, forward/backprop is ~150 lines in one file. Trained via admin-triggered `app/api/pricing-model/train` (matches `valuation-backtest`'s "deliberate action, not background job" pattern), which folds in `meeting_comments.agent_estimate` as lower-weighted (0.3x) soft training examples — this is the literal mechanism for "reinforced by agent feedback," not RLHF (no policy/reward model here, just supervised learning with weighted samples). Holdout accuracy (`pricing_model_runs.holdout_r2`/`holdout_mae_pct`) is logged every run and directly controls how much the blend trusts it (weight = `0.05 + max(0,r2)*0.35`, capped 0.30) — an unproven or bad model barely moves the final number. First real run (2026-07, 813 train / 143 holdout rows, almost entirely government registry since the agency has only 2 closed sales of its own): R²≈0.32, median error≈34% — real signal, not sophisticated yet; expected to improve as `properties`/`meeting_properties` accumulate `balcony`/`utilization_score`/coordinates, which barely exist in the data today (see caveats in `lib/pricingFeatures.ts`). Also surfaces a feature-contribution breakdown (perturb-to-mean, not SHAP) in plain Greek in the reasoning text.
   - **Agent/top-producer feedback** — same as before.
