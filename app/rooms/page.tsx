...
export default function RoomsPage() {
  ...
  async function fetchBookings() {
    const { data } = await supabase.from('room_bookings').select('*, agents(full_name)').eq('date', selectedDate)
    setBookings(data || [])
  }

  function getBooking(room: string, slot: string) { return bookings.find(b => b.room === room && b.time_slot === slot) }

  async function confirmBooking() {
    if (!booking || !clientName || !agent) return
    setLoading(true)
    const { error } = await supabase.from('room_bookings').insert({ room: booking.room, date: selectedDate, time_slot: booking.slot, agent_id: agent.id, client_name: clientName, client_phone: clientPhone })
    if (error) { setToast('Σφάλμα ή ήδη κλεισμένο') } else { setToast('✅ Κλείστηκε!'); setBooking(null); setClientName(''); setClientPhone(''); fetchBookings() }
    setLoading(false); setTimeout(() => setToast(''), 3000)
  }

  const today = new Date().toISOString().split('T')[0]
  const isCeo = agent?.role === 'ceo' || agent?.role === 'admin'
    ...
    <Shell>
      <div style={{ padding: '2rem', maxWidth: 900 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}>
          ...
          <input type="date" value={selectedDate} min={isCeo?undefined:today} onChange={e=>setSelectedDate(e.target.value)}
          ...
          {ROOMS.map(room => (
            <div key={room} style={{ background:'#fff', border:'0.5px solid #e8e8e8', borderRadius:12, padding:'1.25rem' }}>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:12 }}>
                ...
                <div style={{ fontSize:12, color:'#888' }}>{TIME_SLOTS.filter(s=>!getBooking(room,s)).length} διαθέσιμες</div>
              </div>
              <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
                {TIME_SLOTS.map(slot => {
                  ...
                  return (
                    <div key={slot} onClick={()=>!b&&!isPast&&setBooking({room,slot})}
                      style={{ padding:'8px 12px', borderRadius:8, fontSize:13, cursor:b||isPast?'default':'pointer',
                        border: booking?.room===room&&booking?.slot===slot?'1.5px solid #CC2229':b?'0.5px solid #f0c0c0':'0.5px solid #e8e8e8',
                        background: b?(isMine?'#EAF3DE':'#FCEBEB'):isPast?'#f5f5f5':booking?.room===room&&booking?.slot===slot?'#fff5f5':'#fff',
                        color: b?(isMine?'#3B6D11':'#A32D2D'):isPast?'#bbb':'#1a1a1a' }}>
                      <div style={{ display:'flex', justifyContent:'space-between' }}>
                        <span>{slot} – {TIME_SLOTS[TIME_SLOTS.indexOf(slot)+1]||'18:00'}</span>
                        {b && <span style={{ fontWeight:500, fontSize:12 }}>{isMine?`Εσύ · ${b.client_name}`:`${b.agents?.full_name} · ${b.client_name}`}</span>}
        ...
        {booking && (
          <div style={{ marginTop:20, background:'#fff', border:'1.5px solid #CC2229', borderRadius:12, padding:'1.25rem' }}>
            <div style={{ fontSize:15, fontWeight:500, marginBottom:12 }}>{booking.room} — {booking.slot} · {selectedDate}</div>
            <div style={{ display:'flex', gap:10, flexWrap:'wrap', alignItems:'flex-end' }}>
              <div>
                <div style={{ fontSize:12, color:'#888', marginBottom:4 }}>Όνομα πελάτη *</div>
                <input value={clientName} onChange={e=>setClientName(e.target.value)} placeholder="π.χ. Γιώργος Παπαδόπουλος"
              ...
              <div>
                ...
                <input value={clientPhone} onChange={e=>setClientPhone(e.target.value)} placeholder="69xxxxxxxx"
              ...
              <button onClick={()=>setBooking(null)} style={{ padding:'8px 14px', background:'none', border:'0.5px solid #ddd', borderRadius:8, fontSize:13, cursor:'pointer' }}>Ακύρωση</button>
            </div>
          </div>
        )}
        {isCeo && bookings.length > 0 && (
