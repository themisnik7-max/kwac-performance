...
export async function POST(req: NextRequest) {
  ...
  // Verify property belongs to caller's agency
...
export async function DELETE(req: NextRequest) {
  ...
  if (!photo || photo.agency_id !== caller.agency_id) return NextResponse.json({ error: 'Not found' }, { status: 404 })
  if (photo.uploaded_by !== caller.id && caller.role !== 'admin' && caller.role !== 'ceo')
