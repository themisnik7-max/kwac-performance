...
const SECTIONS = [
  ...
  { key: 'listings', label: 'Αναθέσεις', color: '#0F6E56', fields: [
    ...
    { key: 'listings_taken', label: 'Σύνολο Αναθέσεων' },
  ]},
...
export default function SubmitPage() {
  ...
  return (
    <div style={{display:'flex',minHeight:'100vh',background:'#f8f8f7'}}>
      <Sidebar active="submit" role={agent?.role} />
      <main style={{flex:1,padding:'2rem',maxWidth:900}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:'1.25rem'}}>
          <div>
            ...
            <p style={{color:'#888',fontSize:14,margin:'4px 0 0'}}>Εβδομάδα {weekStart} {saved && <span style={{color:'#3B6D11'}}>· Αποθηκεύτηκε {new Date(saved).toLocaleTimeString('el-GR',{hour:'2-digit',minute:'2-digit'})}</span>}</p>
          </div>
          <div style={{display:'flex',alignItems:'center',gap:12}}>
            <div style={{background:'#FAEEDA',color:'#854F0B',borderRadius:8,padding:'6px 14px',fontSize:13,fontWeight:500}}>
              {totalXP} XP
            </div>
            <button onClick={save} disabled={saving} style={{padding:'8px 20px',background:'#CC2229',color:'#fff',border:'none',borderRadius:8,fontSize:13,fontWeight:500,cursor:'pointer'}}>
        ...
        <div style={{display:'flex',gap:16}}>
            ...
            {SECTIONS.map(s => (
              <button key={s.key} onClick={() => setActiveSection(s.key)}
                style={{padding:'8px 12px',borderRadius:8,fontSize:13,cursor:'pointer',textAlign:'left',
                  ...
                  border: activeSection===s.key ? 'none' : '0.5px solid transparent',
          ...
          <div style={{flex:1,background:'#fff',border:'0.5px solid #e8e8e8',borderRadius:12,padding:'1.5rem'}}>
            <div style={{fontSize:15,fontWeight:500,color:activeS.color,marginBottom:1.25+'rem'}}>{activeS.label}</div>
              ...
              {activeS.fields.map(f => (
                <div key={f.key} style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'10px 0',borderBottom:'0.5px solid #f5f5f5'}}>
                  ...
                  <div style={{display:'flex',alignItems:'center',gap:10}}>
                    <button onClick={() => update(f.key,-1)} style={{width:32,height:32,borderRadius:8,border:'0.5px solid #ddd',background:'#f5f5f5',fontSize:16,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center'}}>-</button>
                    <span style={{fontSize:20,fontWeight:500,minWidth:40,textAlign:'center'}}>{values[f.key]||0}</span>
                    <button onClick={() => update(f.key,1)} style={{width:32,height:32,borderRadius:8,border:'0.5px solid #CC2229',background:'#fff5f5',color:'#CC2229',fontSize:16,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center'}}>+</button>
                  </div>
                </div>
              ))}
            </div>
